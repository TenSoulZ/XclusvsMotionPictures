 ...
 * @param {Function} props.onToggleFeatured - Callback for toggling featured status
 * @param {Function} props.onToggleLive - Callback for toggling live status (for live streams)
 * @param {Function} props.onPreview - Callback for preview action (for live streams)
...
const DashboardTable = ({ 
    ...
    if (!data || data.length === 0) {
        return (
            <div className="text-center py-5 text-secondary bg-transparent glass-card border-0">
    ...
    return (
        <div className="glass-card p-0 overflow-hidden border-0 shadow-lg">
            <Table hover variant="dark" responsive className="mb-0 bg-transparent">
                <thead>
                    <tr>
                        <th className="bg-transparent text-secondary">ID</th>
                        {activeTab === 'messages' ? (
                            <>
                                ...
                                <>
                                    <td className="bg-transparent align-middle">
                                        <div style={{ width: '50px', height: '50px', overflow: 'hidden', borderRadius: '8px', background: '#000' }}>
                                            {activeTab === 'photos' ? (
                                                <img src={item.image} alt="thumb" className="w-100 h-100 object-fit-cover" />
                                            ) : (
                                    ...
                                    <td className="bg-transparent align-middle">
                                        <Button variant="link" className="p-0 text-decoration-none" onClick={() => onToggleFeatured(item.id, item.is_featured)}>
                                            {item.is_featured ? (
                                ...
                                <>
                                    <td className="bg-transparent align-middle">
                                        <div style={{ width: '50px', height: '50px', overflow: 'hidden', borderRadius: '8px', background: '#fff', padding: '5px' }}>
                                            <img src={item.logo} alt={item.name} className="w-100 h-100 object-fit-contain" />
                                        </div>
                                    </td>
                                ...
                                <>
                                    <td className="bg-transparent align-middle">
                                        <div className="d-flex align-items-center gap-3">
                                            {item.client_image ? (
                                                <img src={item.client_image} alt={item.client_name} className="rounded-circle object-fit-cover" width="40" height="40" />
                                            ) : (
                                ...
                                <>
                                    <td className="bg-transparent align-middle">
                                        <div style={{ width: '50px', height: '50px', overflow: 'hidden', borderRadius: '8px' }}>
                                            <img src={item.featured_image} alt="preview" className="w-100 h-100 object-fit-cover" />
                                        </div>
                                    </td>
                                    <td className="bg-transparent align-middle fw-bold">{item.title}</td>
                                ...
                                <div className="d-flex justify-content-end gap-2">
                                    {activeTab === 'live' && (
                                        <>
                                            <Button 
                                                variant="link" 
                                                className={`${item.is_live ? 'text-warning' : 'text-success'} p-0 hover-opacity-75`} 
                                            ...
                                            <Button 
                                                variant="link" 
                                                className="text-info p-0 hover-opacity-75" 
                                                title="Preview Feed"
                                    ...
                                    {activeTab !== 'messages' && (
                                        <Button 
                                            variant="link" 
                                            className="text-white p-0 hover-opacity-75" 
                                            title="Edit Details"
                                            onClick={() => onEdit(item)}
                                        >
                                            <FaEdit />
                                    ...
                                    <Button variant="link" className="text-danger p-0 hover-opacity-75" onClick={() => onDelete(item)}>
                                        <FaTrash />
                                    </Button>
                                </div>
                            </td>
